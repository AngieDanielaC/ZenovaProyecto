...
namespace wfZenova
{
    public partial class frmVerCompetencias : Form
    {
        ...
        public frmVerCompetencias(int idDeportista)
        {
            InitializeComponent();
        ...
        private void ConfigurarTablaCompetencias()
        {
            ...
            dgvCompetencias.Columns.Add("Deporte", "DEPORTE");
            dgvCompetencias.Columns.Add("Prueba", "PRUEBA / EVENTO");
            dgvCompetencias.Columns.Add("Puesto", "PUESTO");
            ...
            dgvCompetencias.GridColor = Color.FromArgb(235, 235, 235);
            ...
            dgvCompetencias.ColumnHeadersDefaultCellStyle.BackColor = ColorTranslator.FromHtml("#333FDD");
            dgvCompetencias.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            dgvCompetencias.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI", 10F, FontStyle.Bold);
            ...
            dgvCompetencias.DefaultCellStyle.ForeColor = Color.FromArgb(25, 30, 60);
            dgvCompetencias.DefaultCellStyle.Font = new Font("Segoe UI", 9.5F);
            dgvCompetencias.DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            dgvCompetencias.DefaultCellStyle.SelectionBackColor = Color.FromArgb(240, 242, 255);
            dgvCompetencias.DefaultCellStyle.SelectionForeColor = Color.FromArgb(25, 30, 60);

            dgvCompetencias.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
            dgvCompetencias.Columns["FechaInicio"].FillWeight = 12;
            dgvCompetencias.Columns["FechaFin"].FillWeight = 12;
            dgvCompetencias.Columns["Competencia"].FillWeight = 24;
            dgvCompetencias.Columns["Organizador"].FillWeight = 22;
            dgvCompetencias.Columns["Lugar"].FillWeight = 15;
            dgvCompetencias.Columns["Nivel"].FillWeight = 13;
            dgvCompetencias.Columns["Deporte"].FillWeight = 15;
            dgvCompetencias.Columns["Prueba"].FillWeight = 20;
            dgvCompetencias.Columns["Puesto"].FillWeight = 10;
            ...
            try
            {
                string consulta =
                    @"SELECT DISTINCT D.IdDeporte, D.NombreDeporte
                      ...
                      WHERE PC.IdDeportista = " + idDeportista + @"
                ...
                filaTodos["NombreDeporte"] = "Todos los deportes";
        ...
        private void CargarDatosDeportista()
        {
            try
            {
                string consultaDeportista =
                    @"SELECT IdDeportista, Nombres, Apellidos, Estado
                      FROM Deportistas
                      WHERE IdDeportista = " + idDeportista + @";";

                DataTable dtDeportista = conSQL.RetornaRegistros(consultaDeportista);

                if (dtDeportista == null || dtDeportista.Rows.Count == 0)
                {
                    MessageBox.Show(
                ...
                DataRow fila = dtDeportista.Rows[0];

                lblNombreDeportista.Text = fila["Nombres"].ToString() + " " + fila["Apellidos"].ToString();
                ...
                DataTable dtDeportes = conSQL.RetornaRegistros(consultaDeportes);

                if (dtDeportes != null && dtDeportes.Rows.Count > 0)
                {
                    string deportes = "";

                    foreach (DataRow deporte in dtDeportes.Rows)
                    {
                        if (deportes != "")
                            deportes += ", ";

                        deportes += deporte["NombreDeporte"].ToString();
                    }

                    lblDeporte.Text = "- " + deportes;
                }
                else
                {
                    lblDeporte.Text = "- Sin deporte asignado";
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                string consulta =
                    @"SELECT C.IdCompetencia,
                        ...
                        ISNULL(RC.Prueba, 'Sin registrar') AS Prueba,
                        ISNULL(RC.PuestoObtenido, 'Sin registrar') AS Puesto
                      FROM ParticipantesCompetencia PC
                      INNER JOIN CompetenciaDeporte CD ON PC.IdCompetenciaDeporte = CD.IdCompetenciaDeporte
                      INNER JOIN Competencias C ON CD.IdCompetencia = C.IdCompetencia
                      INNER JOIN Deportes D ON CD.IdDeporte = D.IdDeporte
                      LEFT JOIN ResultadosCompetencia RC ON PC.IdParticipanteCompetencia = RC.IdParticipanteCompetencia
                      WHERE PC.IdDeportista = " + idDeportista + @"
                      ORDER BY C.FechaInicio DESC;";

                dtCompetencias = conSQL.RetornaRegistros(consulta);

                dgvCompetencias.Rows.Clear();

                if (dtCompetencias == null || dtCompetencias.Rows.Count == 0)
                {
                    dgvCompetencias.ClearSelection();
                    return;
                }
                ...
                foreach (DataRow fila in dtCompetencias.Rows)
                {
                    ...
                    if (fila["FechaFin"] != DBNull.Value)
                        fechaFin = Convert.ToDateTime(fila["FechaFin"]).ToString("dd/MM/yyyy");
        ...
        private void CargarTotalCompetencias()
        {
            ...
            {
                string consulta =
                    @"SELECT COUNT(DISTINCT CD.IdCompetencia) AS Total
                      FROM ParticipantesCompetencia PC
                      INNER JOIN CompetenciaDeporte CD ON PC.IdCompetenciaDeporte = CD.IdCompetenciaDeporte
                      WHERE PC.IdDeportista = " + idDeportista + @";";
                ...
                lblTotalCompetencias.Text = "Total de Competencias: " + total;
            }
            catch (Exception ex)
            {
                MessageBox.Show(
        ...
        private void FiltrarCompetencias()
        {
            ...
            string texto = txtBuscarCompetencia.Text.Trim().ToLower();
            string deporteSeleccionado = "";

            if (cmbDeporte.SelectedIndex > 0)
                deporteSeleccionado = cmbDeporte.Text.Trim().ToLower();

            dgvCompetencias.Rows.Clear();

            foreach (DataRow fila in dtCompetencias.Rows)
            {
                string competencia = fila["Competencia"].ToString().ToLower();
                string organizador = fila["Organizador"].ToString().ToLower();
                string lugar = fila["Lugar"].ToString().ToLower();
                string deporte = fila["Deporte"].ToString().ToLower();
                ...
                if (coincideBusqueda && coincideDeporte)
                {
                    string fechaInicio = Convert.ToDateTime(fila["FechaInicio"]).ToString("dd/MM/yyyy");
                    string fechaFin = "";
