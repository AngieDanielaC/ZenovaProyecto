...
namespace wfZenova
{
    public partial class frmVerParticipantes : Form
    {
        ...
        public frmVerParticipantes(int idCompetencia)
        {
            InitializeComponent();
        ...
        public frmVerParticipantes(int idCompetencia, int idEntrenador)
        {
            InitializeComponent();
            this.idCompetencia = idCompetencia;
        ...
        private void ConfigurarTablaParticipantes()
        {
            ...
            dgvParticipantes.Columns.Add("Categoria", "DEPORTE / CATEGORÍA");
            ...
            dgvParticipantes.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(245, 248, 255);
            dgvParticipantes.ColumnHeadersDefaultCellStyle.ForeColor = Color.FromArgb(25, 55, 125);
            ...
            dgvParticipantes.Columns["NombreCompleto"].FillWeight = 60;
            dgvParticipantes.Columns["Categoria"].FillWeight = 40;
        ...
        {
            DataTable tablaCompetencia = conSQL.RetornaRegistros(
                @"SELECT FechaInicio, FechaFin
                  FROM Competencias
                  WHERE IdCompetencia = " + idCompetencia
            );

            if (tablaCompetencia == null || tablaCompetencia.Rows.Count == 0)
            {
                lblFecha.Text = "No disponible";
                return;
            }

            DateTime fechaInicio = Convert.ToDateTime(tablaCompetencia.Rows[0]["FechaInicio"]);
            DateTime fechaFin = Convert.ToDateTime(tablaCompetencia.Rows[0]["FechaFin"]);

            if (fechaInicio.Date == fechaFin.Date)
                lblFecha.Text = fechaInicio.ToString("dd/MM/yyyy");
            else
                lblFecha.Text = fechaInicio.ToString("dd/MM/yyyy") + " - " + fechaFin.ToString("dd/MM/yyyy");
            ...
            else
            {
                consultaDeportes =
                    @"SELECT DISTINCT D.NombreDeporte
                      FROM CompetenciaDeporte CD
                      INNER JOIN Deportes D ON CD.IdDeporte = D.IdDeporte
                      INNER JOIN EntrenadorDeporte ED ON CD.IdDeporte = ED.IdDeporte
                      WHERE CD.IdCompetencia = " + idCompetencia +
            ...
            DataTable tablaDeportes = conSQL.RetornaRegistros(consultaDeportes);

            if (tablaDeportes == null || tablaDeportes.Rows.Count == 0)
            {
                lblDeporte.Text = "Sin deportes";
            }
            else
            {
                string deportes = "";
        ...
        private void CargarParticipantes()
        {
            ...
            if (!esAdministrador)
            {
                filtroEntrenador =
                    ...
                    (
                        ...
                        WHERE I.IdDeportista = DEP.IdDeportista
                        AND I.Estado = 'Activo'
                        AND ED.IdEntrenador = " + idEntrenador.Value + @"
                        AND ED.IdDeporte = CD.IdDeporte
                    )";
            }
            ...
            string consulta =
                @"SELECT DISTINCT
                    DEP.IdDeportista,
                    DEP.Nombres + ' ' + DEP.Apellidos AS NombreCompleto,
                    D.NombreDeporte,
                    (
                        SELECT TOP 1 M.CategoriaEdad
                        FROM MedicionesDeportista M
                        WHERE M.IdDeportista = DEP.IdDeportista
                  ...
                  INNER JOIN Deportistas DEP ON PC.IdDeportista = DEP.IdDeportista
                  INNER JOIN Deportes D ON CD.IdDeporte = D.IdDeporte
                  WHERE CD.IdCompetencia = " + idCompetencia + @"
            ...
            if (tabla == null)
            {
                lblTotalInscritos.Text = "0";
                return;
            }

            foreach (DataRow fila in tabla.Rows)
            {
                string categoria = fila["Categoria"] == DBNull.Value ? "Sin medición" : fila["Categoria"].ToString();
                string categoriaMostrar = fila["NombreDeporte"].ToString();

                if (categoria != "Sin medición")
                    categoriaMostrar += " - " + categoria;
                else
                    categoriaMostrar += " - Sin medición";
            ...
            lblTotalInscritos.Text = dgvParticipantes.Rows.Count.ToString();
            dgvParticipantes.ClearSelection();
        }
